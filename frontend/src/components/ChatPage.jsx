...
const ChatPage = ({ onSettingsOpen }) => {
	...
	useEffect(() => {
		if (user && user._id) {
			...
			// Clear chat state if user logs out
			clearChatState();
		}
	}, [user, initializeSocket, fetchContacts, clearChatState]);


	// Effect to handle selecting the first chat if none is selected
	useEffect(() => {
		if (!selectedChatId && contacts.length > 0) {
			setSelectedChat(contacts[0].id);
		}
	}, [contacts, selectedChatId, setSelectedChat]);


	const handleLogout = () => {
		logout(); // This will also trigger chatStore.clearChatState via the overridden logout
	};
